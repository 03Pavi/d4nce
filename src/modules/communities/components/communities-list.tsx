...
import { Box, Typography, Grid, Card, CardContent, Button, Chip, TextField, InputAdornment, Tabs, Tab, CircularProgress, IconButton } from '@mui/material'
...
export const CommunitiesList = () => {
  ...
  const [membershipStatus, setMembershipStatus] = useState<Record<string, 'pending' | 'approved' | 'rejected'>>({})
  ...
  const handleStartCall = async (selectedUserIds: string[]) => {
    ...
    try {
      ...
      await notifyCallInvite(callerName, selectedCommunity.name, selectedUserIds, roomId);
    ...
    return (
      <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
        <Box sx={{ p: 2, display: 'flex', alignItems: 'center', bgcolor: '#111', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
          <IconButton onClick={() => setSelectedCommunity(null)} sx={{ color: 'white', mr: 1 }}>
            <ArrowBack />
          </IconButton>
          <Box sx={{ flex: 1 }}>
            ...
            <Box sx={{ display: 'flex', gap: 0.5, mt: 0.5 }}>
              {selectedCommunity.tags.map(tag => (
                <Chip key={tag} label={tag} size="small" sx={{ height: 20, fontSize: '0.7rem', bgcolor: 'rgba(255,255,255,0.1)', color: '#ccc' }} />
              ))}
            </Box>
          </Box>
          {viewTab === 1 && (
        ...
        <Box sx={{ flex: 1, overflow: 'hidden', position: 'relative' }}>
          {viewTab === 0 && (
             // @ts-ignore
            <ReelsFeed communityId={selectedCommunity.id} disableUrlSync={true} />
          )}
          {viewTab === 1 && currentUserId && (
            <CommunityChat communityId={selectedCommunity.id} communityName={selectedCommunity.name} currentUserId={currentUserId} />
          )}
          {viewTab === 2 && (
            <Box sx={{ p: 3, color: 'white' }}>
                <Typography variant="h6" sx={{ mb: 2 }}>About {selectedCommunity.name}</Typography>
                <Typography variant="body1" sx={{ color: '#ccc', whiteSpace: 'pre-wrap' }}>
  ...
  return (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', bgcolor: 'black', color: 'white' }}>
      <Box sx={{ p: 2, bgcolor: '#111' }}>
        <Typography variant="h5" sx={{ mb: 2, fontWeight: 'bold', background: 'linear-gradient(45deg, #ff0055, #ff00aa)', backgroundClip: 'text', WebkitBackgroundClip: 'text', color: 'transparent' }}>
          Communities
        </Typography>
        ...
        {loading ? (
          ...
          <Grid container spacing={2}>
                      ...
                      {membershipStatus[community.id] ? (
                        membershipStatus[community.id] === 'approved' ? (
                            <Button 
                              ...
                              sx={{ borderColor: 'rgba(255,0,0,0.5)', color: '#ff4444' }}
                            >
                              Leave
                            </Button>
                        ) : (
                            <Button 
                              ...
                              variant="outlined" 
                              disabled
                              sx={{ borderColor: 'rgba(255,255,255,0.3)', color: '#888' }}
                            >
                              Pending
                            </Button>
                        )
                        ...
                        <Button 
                          ...
                          sx={{ bgcolor: '#ff0055', '&:hover': { bgcolor: '#cc0044' } }}
                        >
                          {community.join_policy === 'approval_required' ? 'Request' : 'Join'}
                        </Button>
                      )}
                    </Box>
            ...
            {filteredCommunities.length === 0 && (
              <Box sx={{ width: '100%', textAlign: 'center', mt: 4, color: '#666' }}>
